...
function saveUserInDB(user, callback) {
    ...
    bcrypt.hash(password, 10, function(err, hash) {
        ...
        mySqlConnection.query(
            registerUserQuery,
            function(result) {
                let user_id = result.results.insertId;
...
function updateUserInfo(userId, body, callback) {
    ...
    let profile_picture = body.profile_picture;
    let gender = body.gender;
    let interest_gender = body.interest_gender;
    let interest_age = body.interest_age;
    let interest_city_id = body.interest_city_id;
...
function updateUserHobby(userId, body, callback) {
    let id = userId;
    let hobby_id = body.Hobby_id;
    let data = [Number(hobby_id), id];
...
function getMatchingProfiles(user, callback) {
    ...
    // GENDER
    let gender = user.gender;
    let interest_gender = user.interest_gender;
    // AGE
    let birth_date = user.birth_date;
    let interest_age_interval = user.interest_age.split('-');
    let min_age = Number(interest_age_interval[0]);
    let max_age = Number(interest_age_interval[1]);
    // calculate min and max birthdate from min and max ages
    let interest_birthdate_min = new Date(
        Date.now() - min_age * 365 * 60 * 24 * 60 * 1000
    );
    let interest_birthdate_max = new Date(
    ...
    let interest_city_id = user.interest_city_id;
