...
const myAuthSqlConnection = require('./mysqlWrapper.js');
...
function saveAccessToken(accessToken, userID, callback) {
    ...
    myAuthSqlConnection.query(getUserQuery, sqlCallback, dataGetuserQuery);
}
...
function getUserAccessToken(userId, callback) {
    ...
    // execute the query to get the user
    myAuthSqlConnection.query(getAccessToken, sqlCallback, dataGetAccessToken[0]);
}
...
function getUserFromAccessToken(userToken, callback) {
    ...
    const userTokenQuery = {
        ...
        sql: 'SELECT usr.*, City.cityname,' +
            ...
            ' WHERE Access_tokens.access_token = ' + myAuthSqlConnection.connection().escape(userToken)
    };
    // holds the results  from the query
    const sqlCallback = dataResponseObject => {
        // calculate if user exists or assign null if results is null
    ...
    // execute the query to get the user
    myAuthSqlConnection.query(userTokenQuery, sqlCallback);
}
